...
class LJ_API_Import extends WP_Importer {
	...
	function greet() {
		...
		<?php else : ?>
			...
			<p><?php _e( 'Enter your LiveJournal username and password below so we can connect to your account:' , 'livejournal-importer') ?></p>
	...
	function setup() {
		...
		if ( empty( $this->username ) || empty( $this->password ) ) {
			?>
			<p><?php _e( 'Please enter your LiveJournal username <em>and</em> password so we can download your posts and comments.' , 'livejournal-importer') ?></p>
		...
		if ( is_wp_error( $verified ) ) {
			if ( 100 == $this->ixr->getErrorCode() || 101 == $this->ixr->getErrorCode() ) {
				...
				?>
				<p><?php _e( 'Logging in to LiveJournal failed. Check your username and password and try again.' , 'livejournal-importer') ?></p>
				<p><a href="<?php echo esc_url($_SERVER['PHP_SELF'] . '?import=livejournal&amp;step=-1&amp;_wpnonce=' . wp_create_nonce( 'lj-api-import' ) . '&amp;_wp_http_referer=' . esc_attr( str_replace( '&step=1', '', $_SERVER['REQUEST_URI'] ) ) ) ?>"><?php _e( 'Start again' , 'livejournal-importer') ?></a></p>
				<?php
				return false;
			} else {
	...
	function auto_submit( $id = 'ljapi-next-form', $msg = 'auto-message', $seconds = 10 ) {
		?><script type="text/javascript">
			...
			function ljapi_msg() {
				str = '<?php _e( "Continuing in %d&#8230;" , 'livejournal-importer') ?>';
				jQuery( '#<?php echo $msg ?>' ).text( str.replace( /%d/, next_counter ) );
				if ( next_counter <= 0 ) {
					if ( jQuery( '#<?php echo $id ?>' ).length ) {
						jQuery( "#<?php echo $id ?> input[type='submit']" ).hide();
						str = '<?php _e( "Continuing&#8230;" , 'livejournal-importer') ?> <img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" alt="" id="processing" align="top" />';
			...
			function ljapi_msg() {
				...
				if ( next_counter <= 0 ) {
					if ( jQuery( '#<?php echo $id ?>' ).length ) {
						...
						str = '<?php _e( "Processing next batch." , 'livejournal-importer') ?> <img src="<?php echo esc_url( admin_url( 'images/wpspin_light.gif' ) ); ?>" alt="" id="processing" align="top" />';
						jQuery( '#<?php echo $msg ?>' ).html( str );
